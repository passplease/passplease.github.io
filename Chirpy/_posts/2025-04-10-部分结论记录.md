...
## 可构造集合公理
此公理假设论域中全部集合都是可构造的，即$V=L$，其中$V$表示全体集合组成的论域，$L$则是全体可定义集合组成的论域（可定义集合见[概念辨析](../集合论第二卷概念辨析/#mathfrak-d)）。
>此外补充,$L[A]$表示在定义$L$的时候多引入一个谓词表示是否属于$A$，$L(A)$则更强，为$\operatorname{TC}(\{A\})$的全部集合都引入一个谓词，且第一层就直接从$\operatorname{TC}(\{A\})$开始
{: .prompt-info }
...
### 可数个稠密子集的泛型滤子存在定理
定理3.1<br>
设$\mathbb{P} = (P, <, 1)$是一个力迫构思。设$\mathcal{D}$是$\mathbb{P}$的稠密开子集的一个可数集合。那么，任给$\mathbb{P}$中的一个条件$p$必有$\mathbb{P}$的一个包括此条件$p$的$\mathcal{D}$泛型滤子$F$<br><br>
证明过程就是根据可数性一个个选。这个定理的重要性在于对于我们构造力迫构思所在的模型，如果它能够是一个可数模型，这个模型中的所有力迫构思都有对应的泛型滤子存在，因为其是可数的，我们不必被泛型滤子的存在性挡住去路。
### 真相引理
...
## 基数大小比较
定义部分可以查看[概念辨析](../集合论第三卷概念辨析/#大基数)<br><br>
不可达基数<拉姆齐基数<可测基数<强紧基数<超紧基数

## 强紧基数
定义可见[第三卷概念辨析](../集合论第三卷概念辨析/#超紧基数)，共三种等价定义，第三种等价定义是
- 任何一个集合$X$上的每一个$\kappa-$完全的滤子都可以扩展为一个$\kappa-$完全的超滤子
