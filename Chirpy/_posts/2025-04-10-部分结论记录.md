...
设$\omega\leq\kappa$为一个基数。$\mathrm{MA}_\kappa$（马丁公理）是后述命题：如果$(P,<)$是一个满足可数反链条件的偏序集合，$\mathscr C$是$(P,<)$的不超过$\kappa$个稠密子集的集合，那么一定存在$(P,<)$的一个$\mathscr C$-泛善子集$F$<br><br>
...
---
马丁公理独立于ZFC公理体系，如果$\mathbf{MA}_{\omega_1}$成立，那么则没有苏斯林树；而如果苏斯林树存在，$\mathbf{MA}_{\omega_1}$也不存在，两者是等价的。
