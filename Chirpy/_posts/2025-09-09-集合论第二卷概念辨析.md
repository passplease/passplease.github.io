...
2. $(p_{1},\dot{q}_{1})\leqslant_{P*\dot{Q}}(p_{2},\dot{q}_{2})\iff [p_{1}\leqslant_{P}p_{2}\land p_{1}\Vdash \dot{q}_{1}\leqslant_{Q}\dot{q}_{2}]$
...
1. 一个长度为$\alpha +1$的双序列
	...
	4. $\mathbf{1}_{\alpha +1} \upharpoonright_{\alpha} = \mathbf{1}_{\alpha}$以及$\mathbf{1}_{\alpha +1}(\alpha) = \dot{\mathbf{1}}_{\alpha}$
...
2. 对于极限序数$\alpha$而言，一个长度为$\alpha$的双序列
	...
	2. $\forall p(p \in P_{\alpha} \leftrightarrow (\mathrm{dom}(p) = \alpha \wedge \forall \beta < \alpha (p \upharpoonright_{\beta} \in P_{\beta}) \wedge |\mathrm{spt}(p)| < \omega))$，其中

		$$
		\mathrm{spt}(p) = \{\beta < \alpha \mid \mathbf{1}_{\beta} \mid \forall p(\beta) = \dot{\mathbf{1}}_{\beta}\}
		$$

	3. $\forall \beta < \alpha$ $(\mathbf{1}_{\alpha} \upharpoonright_{\beta} = \mathbf{1}_{\beta})$
