...
### 解决
看了书中的证明之后，我认为书中对于$\mathscr E_\omega(\kappa)$的定义是不对的，书中证明取的左边的元素就是普通的$\omega$-无界闭子集，并没有强调其只含有极限序数，和其给出的定义是不相符的；此外这个的定义和之前定义$\mathscr E(\kappa)$也不一样，$\mathscr E(\kappa)$收集了所有包含无界闭子集的集合，但是此处的定义却截然不同<br><br>
