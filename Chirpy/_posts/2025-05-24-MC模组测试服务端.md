...
首先最需要更改的是服务器设置，文件是`run/server.properties`{: .filepath}[^path]，其中我们需要更改连接的玩家的限制，允许离线账号登录，否则`runClient`启动的游戏是无法登录进服务器的。具体地是`online-mode`条我们需要设置为`true`，设置之后我们就可以加入服务器了。<br><br>
...
此外我在运行`runGameTestServer`时遇到了运行失败的问题，原因是机械动力有自定义的需要在`runGameTestServer`时运行的测试，而我并没有包含其导致失败，我的解决办法很奇怪，我发现我的`build.gradle`{: .filepath}[^path]文件最开始写的是
```groovy
runs{
    gameTestServer {
