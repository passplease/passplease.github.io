...
---
因为最近关于[我的一个项目](../B站推荐器开发/)有架构变动的情况，所以我第一次在电脑上安装Docker，考虑到最后我的代码也要在Docker环境中测试，所以我想，索性平常就直接在Docker环境中测试，将Docker配置到CLion的Toolchain中，不仅方便测试，而且适合代码实际运行环境。下面就是配置的流程以及遇到的一些问题和解决方法。（全程都在Gemini-2.5-pro辅助下完成）
